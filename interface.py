...
st.set_page_config(page_title="Bozok Deepfake AI - Xception Ultra", layout="wide")
...
st.markdown("<h1 style='text-align: center; color: #1B365D;'>🛡️ Xception Tabanlı Deepfake Görüntü Analiz Paneli</h1>", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    with col1:
        ...
        if st.button("🚀 Derin Analizi Başlat (20 Kare Modu)", use_container_width=True):
            ...
            if avg_prob > 0.5:
                # Rastgele değil, en yüksek riskli karedeki Grad-CAM odak varyansına göre teşhis simülasyonu yapıyoruz
...
Kullanılan Model   : Xception Detector Backbone
...
=================================================="""
            ...
            # --- ANLIK KARE BAZLI GÜVEN GRAFİĞİ ---
            st.write("---")
            st.write("**📈 Kare Bazlı Kronolojik Sahtelik Dağılım Grafiği:**")
            fig, ax = plt.subplots(figsize=(6, 2.3))
            plot_color = '#ff4d4d' if fake_percentage > 50 else '#0099cc'
            ax.plot(range(1, len(current_probs) + 1), current_probs, marker='o', color=plot_color, linewidth=2)
            ax.axhline(y=0.5, color='gray', linestyle='--')
            ax.set_ylim([0, 1.05])
            ax.set_xlabel('Yüz Kare Numarası')
            ax.set_ylabel('Fake Olasılığı')
            ax.grid(alpha=0.2)
            plt.tight_layout()
            st.pyplot(fig)
            plt.close()
